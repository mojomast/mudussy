...
export class ConfigValidationService {
  ...
  private validateCriticalConfig(result: ValidationResult) {
  ...
  private validateEnvironmentConfig(result: ValidationResult) {
  ...
  private validateNetworkingConfig(result: ValidationResult) {
  ...
  private validateDatabaseConfig(result: ValidationResult) {
    const dbType = this.configService.get('MUD_DATABASE_TYPE', 'redis') as string;
  ...
  private validateSecurityConfig(result: ValidationResult) {
    const env = process.env.NODE_ENV;
    ...
    if (env === 'production') {
      const sessionSecret = this.configService.get('MUD_SESSION_SECRET', '') as string;
  ...
  private validatePerformanceConfig(result: ValidationResult) {
