// World Visualization Components for Interactive World Management
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className="w-full px-3 py-2 bg-mud-black border border-mud-green text-mud-green rounded focus:outline-none focus:border-mud-light"
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            rows="3"
                            className="w-full px-3 py-2 bg-mud-black border border-mud-green text-mud-green rounded focus:outline-none focus:border-mud-light resize-none"
                            disabled={user.role !== 'admin'}
                        />
                    </div>
                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                            {Object.entries(editData.exits).map(([direction, destination]) => (
                                <div key={direction} className="flex justify-between items-center bg-mud-black border border-mud-green p-2 rounded">
                                    <span className="text-mud-green capitalize">{direction} → {destination}</span>
                                    {user.role === 'admin' && (
                                        <button
                                            onClick={() => removeExit(direction)}
                        ...
                        {user.role === 'admin' && (
                            <div className="flex space-x-2 mt-2">
                                <input
                                    ...
                                    className="flex-1 px-3 py-2 bg-mud-black border border-mud-green text-mud-green rounded focus:outline-none focus:border-mud-light"
                                />
                                <input
                                    type="text"
                                    placeholder="Room ID"
                                    value={newExitRoomId}
                                    onChange={(e) => setNewExitRoomId(e.target.value)}
                                    className="flex-1 px-3 py-2 bg-mud-black border border-mud-green text-mud-green rounded focus:outline-none focus:border-mud-light"
                                />
                                <button
                                    onClick={addExit}
                                    className="bg-mud-green text-mud-black px-4 py-2 rounded font-bold hover:bg-mud-light"
                                >
                                    Add
                                </button>
                            </div>
                    ...
                    <div className="flex justify-end space-x-2 pt-4">
                        <button
                            onClick={onClose}
                            className="px-4 py-2 bg-mud-dark border border-mud-green text-mud-green rounded hover:bg-gray-700"
                        >
                            Cancel
                        </button>
                        {user.role === 'admin' && (
                            <button
                                onClick={handleSave}
                                disabled={saving}
                                className="px-4 py-2 bg-mud-green text-mud-black rounded font-bold hover:bg-mud-light disabled:opacity-50"
                            >
...
// Entity Management Modal Component
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            placeholder="Room ID to move entity to"
                            className="w-full px-3 py-2 bg-mud-black border border-mud-green text-mud-green rounded focus:outline-none focus:border-mud-light"
                        />
                    </div>
                    ...
                    {user.role === 'admin' && (
                        <div className="flex flex-wrap gap-2 pt-4">
                            <button
                                ...
                                className="flex-1 bg-green-600 text-white px-3 py-2 rounded font-bold hover:bg-green-500"
                            >
                                Spawn Entity
                            </button>
                            <button
                                onClick={() => handleAction('move')}
                                className="flex-1 bg-blue-600 text-white px-3 py-2 rounded font-bold hover:bg-blue-500"
                            >
                                Move Entity
                            </button>
                            <button
                                onClick={() => handleAction('despawn')}
                                className="flex-1 bg-red-600 text-white px-3 py-2 rounded font-bold hover:bg-red-500"
                            >
                                Despawn Entity
                            </button>
                        </div>
...
// World Map Component - Displays rooms in a grid layout
function WorldMap({ rooms, selectedRoom, onRoomSelect, connections, onRoomEdit, onEntityManage, user }) {
    ...
    const getRoomPosition = (room) => {
        if (!room.coordinates) return { x: 0, y: 0 };
        ...
        return { x: 0, y: 0 }; // Tree layout would be calculated differently
    };
    ...
    const renderConnections = () => {
        ...
        return connections.map((connection, index) => {
            ...
            if (!fromRoom || !toRoom || !fromRoom.coordinates || !toRoom.coordinates) return null;
    ...
    const renderRoom = (room) => {
        ...
        return (
            <g key={room.id}>
                ...
                {user.role === 'admin' && isSelected && (
                    <g>
                        <circle
                            ...
                            r="8"
                            fill="#22c55e"
                            className="cursor-pointer hover:fill-mud-light"
                            onClick={(e) => {
                                e.stopPropagation();
                ...
                {/* Manage Entities Button for Admins */}
                {user.role === 'admin' && isSelected && (
                    <g>
                        <circle
                            cx={position.x + 100}
    ...
    return (
        <div className="bg-mud-black border-2 border-mud-green rounded-lg p-4">
            {/* Controls */}
            <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex items-center space-x-2">
                    ...
                    <button
                        ...
                        className="px-2 py-1 bg-mud-dark border border-mud-green text-mud-green rounded text-sm"
                    >
                        -
                    </button>
...
function RoomDetailPanel({ room, npcs, items, onClose }) {
    ...
    return (
        <div className="bg-mud-dark border-2 border-mud-green rounded-lg p-4 max-w-md">
            <div className="flex justify-between items-center mb-4">
            ...
            <div className="space-y-4">
                ...
                <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        {Object.entries(room.exits || {}).map(([direction, destination]) => (
                            <div key={direction} className="bg-mud-black border border-mud-green p-2 rounded text-sm">
                ...
                <div>
                    ...
                    <div className="space-y-2">
                        {roomNpcs.map(npc => (
                            <div key={npc.id} className="bg-mud-black border border-mud-green p-2 rounded">
                                ...
                                <div className="flex justify-between text-xs mt-1">
                                    ...
                                    <span className={`capitalize ${
                                        npc.stats.aggression === 'friendly' ? 'text-green-400' :
                        ...
                        {roomNpcs.length === 0 && (
                            <p className="text-mud-light text-sm">No NPCs in this room</p>
                        )}
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div className="space-y-2">
                        {roomItems.map(item => (
                            <div key={item.id} className="bg-mud-black border border-mud-green p-2 rounded">
                                <div className="text-mud-green font-bold">{item.name}</div>
                                <div className="text-mud-light text-sm">{item.description}</div>
                            </div>
                        ))}
                        {roomItems.length === 0 && (
                            <p className="text-mud-light text-sm">No items in this room</p>
                        )}
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div className="space-y-2">
                        {room.players?.map(player => (
                            <div key={player} className="bg-mud-black border border-mud-green p-2 rounded">
                                <div className="text-green-400 font-bold">{player}</div>
                                <div className="text-mud-light text-sm">Online</div>
                            </div>
                        ))}
                        {(!room.players || room.players.length === 0) && (
                            <p className="text-mud-light text-sm">No players in this room</p>
                        )}
                    </div>
                </div>
            </div>
...
function SearchAndFilter({ onSearch, onFilter }) {
    ...
    return (
        <div className="bg-mud-dark border border-mud-green rounded-lg p-4 mb-4">
            <div className="flex flex-wrap gap-4">
                ...
                <div className="flex items-center space-x-4">
                    <label className="flex items-center space-x-2 text-sm">
                        <input
                            type="checkbox"
                            checked={filters.hasNpcs}
                            onChange={(e) => handleFilterChange('hasNpcs', e.target.checked)}
                            className="bg-mud-black border border-mud-green text-mud-green"
                        />
                    ...
                    <label className="flex items-center space-x-2 text-sm">
                        <input
                            ...
                            onChange={(e) => handleFilterChange('hasItems', e.target.checked)}
                            className="bg-mud-black border border-mud-green text-mud-green"
                        />
                    ...
                    <label className="flex items-center space-x-2 text-sm">
                        <input
                            ...
                            onChange={(e) => handleFilterChange('hasPlayers', e.target.checked)}
                            className="bg-mud-black border border-mud-green text-mud-green"
                        />
...
// NPC Location Tracker Component
        ...
        <div className="bg-mud-dark border border-mud-green rounded-lg p-4">
            <div className="flex justify-between items-center mb-4">
                ...
                <select
                    ...
                    className="px-3 py-1 bg-mud-black border border-mud-green text-mud-green rounded text-sm"
                >
...
function ItemDistribution({ items, rooms, onItemSelect }) {
    ...
    return (
        <div className="bg-mud-dark border border-mud-green rounded-lg p-4">
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
                {sortedRooms.map(([roomId, data]) => (
                    <div key={roomId} className="bg-mud-black border border-mud-green rounded p-3">
                ...
                {sortedRooms.length === 0 && (
                    <p className="text-mud-light text-center py-4">No items found in rooms</p>
                )}
            </div>
        </div>
    );
...
function AccessibilityPanel({ isOpen, onClose, user }) {
    if (!isOpen || user.role !== 'moderator') return null;
...
function WorldVisualization({ api, user }) {
    ...
    const handleEntityAction = async (action, entityType, entityId, targetRoomId) => {
        ...
        switch (action) {
            ...
            case 'spawn':
                if (entityType === 'npc') {
                    // Create new NPC in the managing room
                    ...
                    // Create new item in the managing room
    ...
    const filteredRooms = rooms.filter(room => {
        ...
        if (searchTerm) {
            ...
            const roomNpcs = npcs.filter(npc => npc.location === room.id);
            const hasMatchingNpc = roomNpcs.some(npc =>
        ...
        // Other filters
        if (filters.hasNpcs && (!room.npcs || room.npcs.length === 0)) return false;
        if (filters.hasItems && (!room.items || room.items.length === 0)) return false;
        if (filters.hasPlayers && (!room.players || room.players.length === 0)) return false;
            ...
            <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
                ...
                <div className="xl:col-span-2">
                    <WorldMap
                        ...
                        onEntityManage={handleEntityManage}
                        user={user}
                    />
                </div>

                {/* Side Panel - Takes up 2 columns on xl screens */}
