...
class ApiService {
    constructor() {
        // Always target the server API under the /api prefix on the same origin
...
function LoginForm({ onLogin }) {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-mud-black">
            <div className="bg-mud-dark border-2 border-mud-green p-8 rounded-lg shadow-lg w-96">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full bg-mud-green text-mud-black py-2 px-4 rounded font-bold hover:bg-mud-light disabled:opacity-50 disabled:cursor-not-allowed"
                    >
...
function App() {
    ...
    return (
        <div className="min-h-screen bg-mud-black text-mud-green">
            <AdminPanel
                user={user}
...
function AdminPanel({ user, currentTab, onTabChange, onLogout, api }) {
    ...
    const getAvailableTabs = () => {
        ...
        if (user.role === 'admin' || user.role === 'moderator') {
            tabs.push(
                { id: 'users', label: 'Users', icon: '👥', roles: ['admin', 'moderator'] },
                { id: 'world', label: 'World', icon: '🌍', roles: ['admin', 'moderator'] },
                { id: 'dialogue', label: 'Dialogue', icon: '💬', roles: ['admin', 'moderator'] }
            );
        }
