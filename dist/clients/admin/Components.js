...
const SecurityUtils_js_1 = require("./SecurityUtils.js");
function DashboardTab({ api, user }) {
    const permissions = (0, SecurityUtils_js_1.usePermissions)(user);
    ...
    return (<div className="space-y-6">
            ...
            <div className="bg-mud-dark border border-mud-green p-4 rounded-lg">
                ...
                <div className="space-y-2">
                    {stats?.recentActivity?.map((activity, index) => (<div key={activity.id || index} className="flex justify-between items-center py-2 border-b border-mud-black last:border-b-0">
                            <div>
                        ...
                        </div>)) || <p className="text-mud-light">No recent activity</p>}
                </div>
            </div>
        </div>);
}
function UsersTab({ api, user }) {
    ...
    return (<div className="space-y-6">
            <div className="flex justify-between items-center">
                ...
                {permissions.canManageUsers() && (<button className="bg-mud-green text-mud-black px-4 py-2 rounded font-bold hover:bg-mud-light">
                        Add New User
                    </button>)}
            </div>
            ...
            <div className="bg-mud-dark border border-mud-green rounded-lg overflow-hidden">
                <table className="w-full">
                    ...
                    <tbody>
                        {users.map((user) => (<tr key={user.id} className="border-t border-mud-black hover:bg-mud-black">
                                <td className="px-4 py-2 text-mud-light">{user.username}</td>
                                <td className="px-4 py-2">
                                    <span className={`px-2 py-1 rounded text-xs font-bold ${user.role === 'admin' ? 'bg-red-900 text-white' :
                user.role === 'moderator' ? 'bg-yellow-600 text-black' :
                    'bg-mud-green text-mud-black'}`}>
                                ...
                                <td className="px-4 py-2">
                                    <span className={`px-2 py-1 rounded text-xs font-bold ${user.isActive ? 'bg-green-600 text-white' : 'bg-red-600 text-white'}`}>
                                ...
                                <td className="px-4 py-2 space-x-2">
                                    {user.isActive ? (<button onClick={() => handleUserAction('deactivate', user.id)} className="bg-red-600 hover:bg-red-500 text-white px-2 py-1 rounded text-xs font-bold">
                                            Deactivate
                                        </button>) : (<button onClick={() => handleUserAction('activate', user.id)} className="bg-green-600 hover:bg-green-500 text-white px-2 py-1 rounded text-xs font-bold">
                                            Activate
                                        </button>)}
                                    {permissions.canManageUsers() && (<button onClick={() => handleUserAction('resetPassword', user.id)} className="bg-yellow-600 hover:bg-yellow-500 text-black px-2 py-1 rounded text-xs font-bold">
                                            Reset Password
                                        </button>)}
                                </td>
                            </tr>))}
    ...
    return (<div className="space-y-6">
            <div className="flex justify-between items-center">
                ...
                <div className="flex space-x-2">
                    <button onClick={() => setActiveView('visualization')} className={`px-4 py-2 rounded font-bold ${activeView === 'visualization'
            ? 'bg-mud-green text-mud-black'
            : 'bg-mud-dark text-mud-green border border-mud-green'}`}>
                        Interactive Map
                    </button>
                    <button onClick={() => setActiveView('management')} className={`px-4 py-2 rounded font-bold ${activeView === 'management'
            ? 'bg-mud-green text-mud-black'
            : 'bg-mud-dark text-mud-green border border-mud-green'}`}>
                        Management
                    </button>
            ...
            {activeView === 'visualization' ? (<WorldVisualization api={api} user={user}/>) : (<div className="space-y-6">
                    ...
                    <div className="bg-mud-dark border border-mud-green p-4 rounded-lg">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {worldData?.sectors?.map((sector) => (<div key={sector.id} className="bg-mud-black border border-mud-green p-4 rounded">
                                ...
                                </div>)) || <p className="text-mud-light">No sectors loaded</p>}
                        </div>
                    </div>

                    
                    {permissions.canManageWorld() && (<div className="bg-mud-dark border border-mud-green p-4 rounded-lg">
...
function DialogueTreeView({ tree, onNodeSelect, selectedNodeId, onNodeToggle, expandedNodes }) {
    ...
    const getNodePosition = (nodeId, level = 0, visited = new Set()) => {
        ...
        const children = connections.map(childId => getNodePosition(childId, level + 1, new Set(visited))).filter(Boolean);
    ...
    const renderNode = (nodeId, level = 0) => {
        ...
        const connections = getNodeConnections(nodeId);
        return (<div key={nodeId} className="mb-2">
                <div className={`flex items-center p-2 rounded border cursor-pointer transition-colors ${isSelected
                ? 'bg-mud-green text-mud-black border-mud-light'
                : 'bg-mud-dark text-mud-light border-mud-green hover:bg-mud-black'} ${dragOver === nodeId ? 'ring-2 ring-mud-light' : ''}`} style={{ marginLeft: `${level * 20}px` }} onClick={() => onNodeSelect(nodeId)} draggable onDragStart={(e) => {
                setDraggedNode(nodeId);
                e.dataTransfer.effectAllowed = 'move';
            }} onDragOver={(e) => {
                e.preventDefault();
                setDragOver(nodeId);
            }} onDragLeave={() => setDragOver(null)} onDrop={(e) => {
                e.preventDefault();
                ...
                setDragOver(null);
                    ...
                    <button className="mr-2 text-mud-green hover:text-mud-light" onClick={(e) => {
                e.stopPropagation();
                onNodeToggle(nodeId);
            }}>
    ...
    return (<div className="bg-mud-black border border-mud-green rounded-lg p-4 max-h-96 overflow-y-auto">
...
function DialogueNodeEditor({ node, tree, onNodeUpdate, onSave, onCancel, user, validationErrors = [] }) {
    ...
    return (<div className="bg-mud-dark border border-mud-green rounded-lg p-4">
            <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => setShowAdvanced(!showAdvanced)} className="text-sm text-mud-light hover:text-mud-green">
            ...
            <div className="space-y-4">
                <div>
                    ...
                    <input type="text" value={editedNode.id} onChange={(e) => handleNodeChange('id', e.target.value)} className={`w-full px-3 py-2 bg-mud-black border text-mud-green rounded focus:outline-none ${getValidationError('id') ? 'border-red-500' : 'border-mud-green focus:border-mud-light'}`} placeholder="e.g., blacksmith_greeting"/>
                    {getValidationError('id') && (<div className="text-red-400 text-xs mt-1">{getValidationError('id').message}</div>)}
                </div>

                <div>
                    <label className="block text-sm font-bold text-mud-green mb-1">NPC Message</label>
                    <textarea value={editedNode.text} onChange={(e) => handleNodeChange('text', e.target.value)} rows={4} className={`w-full px-3 py-2 bg-mud-black border text-mud-green rounded focus:outline-none resize-vertical ${getValidationError('text') ? 'border-red-500' : 'border-mud-green focus:border-mud-light'}`} placeholder="Enter the NPC's dialogue text..."/>
                    {getValidationError('text') && (<div className="text-red-400 text-xs mt-1">{getValidationError('text').message}</div>)}
                </div>

                <div className="flex items-center space-x-2">
                    <input type="checkbox" id="isEndNode" checked={editedNode.isEndNode} onChange={(e) => handleNodeChange('isEndNode', e.target.checked)} className="bg-mud-black border-mud-green text-mud-green focus:ring-mud-light"/>
                ...
                <div>
                    ...
                    <div className="space-y-2 max-h-48 overflow-y-auto">
                        {editedNode.responses.map((choice, index) => (<div key={choice.id} className="bg-mud-black border border-mud-green rounded p-3">
                                <div className="flex justify-between items-start mb-2">
                                    <input type="text" value={choice.text} onChange={(e) => handleChoiceChange(index, 'text', e.target.value)} className="flex-1 px-3 py-1 bg-mud-dark border border-mud-green text-mud-green rounded focus:outline-none focus:border-mud-light mr-2" placeholder="Choice text..."/>
                                    <button onClick={() => removeChoice(index)} className="text-red-400 hover:text-red-300 text-sm px-2 py-1">
                                        ✕
                                    </button>
                                </div>
                                <div className="flex items-center space-x-2">
                                    <label className="text-xs text-mud-light">Next Node:</label>
                                    <select value={choice.nextNodeId || ''} onChange={(e) => handleChoiceChange(index, 'nextNodeId', e.target.value)} className="px-2 py-1 bg-mud-dark border border-mud-green text-mud-green rounded text-xs focus:outline-none focus:border-mud-light">
                                        <option value="">End Conversation</option>
                                        {Object.keys(tree.nodes).map(nodeId => (<option key={nodeId} value={nodeId}>{nodeId}</option>))}
                                    </select>
                                </div>
                            </div>))}
                    </div>
                    ...
                    <div className="mt-2 p-2 bg-mud-black border border-mud-green rounded">
                        <div className="flex space-x-2">
                            <input type="text" value={newChoice.text} onChange={(e) => setNewChoice(prev => ({ ...prev, text: e.target.value }))} className="flex-1 px-3 py-1 bg-mud-dark border border-mud-green text-mud-green rounded focus:outline-none focus:border-mud-light" placeholder="New choice text..."/>
                            <select value={newChoice.nextNodeId} onChange={(e) => setNewChoice(prev => ({ ...prev, nextNodeId: e.target.value }))} className="px-2 py-1 bg-mud-dark border border-mud-green text-mud-green rounded text-xs focus:outline-none focus:border-mud-light">
                                <option value="">End Conversation</option>
                                {Object.keys(tree.nodes).map(nodeId => (<option key={nodeId} value={nodeId}>{nodeId}</option>))}
                            </select>
                            <button onClick={addChoice} className="bg-mud-green text-mud-black px-3 py-1 rounded text-sm font-bold hover:bg-mud-light">
                                Add
                            </button>
                        </div>
                    </div>
                ...
                {showAdvanced && (<div className="border-t border-mud-green pt-4">
            ...
            <div className="flex justify-end space-x-2 mt-6 pt-4 border-t border-mud-green">
                <button onClick={onCancel} className="px-4 py-2 border border-mud-green text-mud-green rounded hover:bg-mud-green hover:text-mud-black">
                    Cancel
                </button>
                <button onClick={() => onSave(editedNode)} className="px-4 py-2 bg-mud-green text-mud-black rounded font-bold hover:bg-mud-light" disabled={!editedNode.id.trim() || !editedNode.text.trim()}>
...
function DialogueTab({ api, user }) {
    ...
    return (<div className="space-y-6">
            <div className="flex justify-between items-center">
                ...
                <div className="flex space-x-2">
                    <button onClick={() => setActiveView('trees')} className={`px-4 py-2 rounded font-bold ${activeView === 'trees'
            ? 'bg-mud-green text-mud-black'
            : 'bg-mud-dark text-mud-green border border-mud-green'}`}>
                        Trees
                    </button>
                    <button onClick={() => setActiveView('editor')} className={`px-4 py-2 rounded font-bold ${activeView === 'editor'
            ? 'bg-mud-green text-mud-black'
            : 'bg-mud-dark text-mud-green border border-mud-green'}`}>
                        Editor
                    </button>
                    {permissions.canManageDialogue() && (<button className="bg-mud-green text-mud-black px-4 py-2 rounded font-bold hover:bg-mud-light">
                            Create New Tree
                        </button>)}
                </div>
            </div>

            {activeView === 'trees' && (<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    
                    <div className="lg:col-span-1 space-y-4">
                        <h3 className="text-lg font-bold text-mud-green">Dialogue Trees</h3>
                        <div className="space-y-2 max-h-96 overflow-y-auto">
                            {dialogueTrees.map((tree) => (<div key={tree.id} onClick={() => handleTreeSelect(tree)} className={`p-3 rounded border cursor-pointer transition-colors ${selectedTree?.id === tree.id
                    ? 'bg-mud-green text-mud-black border-mud-light'
                    ...
                    <div className="lg:col-span-2">
                        {selectedTree ? (<div className="space-y-4">
                                <div className="flex justify-between items-center">
                                    <h3 className="text-lg font-bold text-mud-green">Tree Structure</h3>
                                    <div className="flex space-x-2">
                                        <button onClick={createNewNode} className="bg-mud-green text-mud-black px-3 py-1 rounded text-sm font-bold hover:bg-mud-light">
                                            Add Node
                                        </button>
                                        <button onClick={() => validateDialogueTree(selectedTree.id)} className="bg-blue-600 text-white px-3 py-1 rounded text-sm font-bold hover:bg-blue-500">
                                            Validate
                                        </button>
                                        {permissions.canManageDialogue() && unsavedChanges && (<button onClick={() => saveDialogueTree(selectedTree)} className="bg-yellow-600 text-black px-3 py-1 rounded text-sm font-bold hover:bg-yellow-500">
                                                Save Changes
                                            </button>)}
                                    </div>
                                </div>

                                <DialogueTreeView tree={selectedTree} onNodeSelect={handleNodeSelect} selectedNodeId={selectedNodeId} onNodeToggle={handleNodeToggle} expandedNodes={expandedNodes}/>

                                
                                {validationErrors.length > 0 && (<div className="bg-red-900 border border-red-600 rounded p-3">
                                        <h4 className="text-red-400 font-bold mb-2">Validation Issues:</h4>
                                        <ul className="text-sm text-red-300 space-y-1">
                                            {validationErrors.map((error, index) => (<li key={index}>• {error}</li>))}
                                        </ul>
                                    </div>)}
                            </div>) : (<div className="text-center py-8 text-mud-light">
                                Select a dialogue tree to view its structure
                            </div>)}
                    </div>
                </div>)}

            {activeView === 'editor' && showNodeEditor && editingNode && (<div className="max-w-4xl">
                    <DialogueNodeEditor node={editingNode} tree={selectedTree} onNodeUpdate={handleNodeSave} onSave={handleNodeSave} onCancel={() => {
                setShowNodeEditor(false);
                setEditingNode(null);
                setActiveView('trees');
            }} user={user} validationErrors={validationErrors}/>
                </div>)}

            {activeView === 'editor' && !showNodeEditor && (<div className="text-center py-8 text-mud-light">
            ...
            <div className="bg-mud-dark border border-mud-green p-4 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <button onClick={() => selectedTree && validateDialogueTree(selectedTree.id)} className="bg-mud-green text-mud-black p-4 rounded font-bold hover:bg-mud-light text-left">
...
function AdminPanel({ user, currentTab, onTabChange, onLogout, api }) {
    ...
    return (<div className="relative min-h-screen">
            ...
            {permissions.canSystemAdmin() && (<button onClick={() => setIsAdminPanelOpen(!isAdminPanelOpen)} className="fixed bottom-6 right-6 w-14 h-14 bg-mud-green text-mud-black rounded-full shadow-lg hover:bg-mud-light transition-colors flex items-center justify-center text-2xl font-bold z-50" title="Admin Settings">
                    ⚙️
                </button>)}

            
            {isAdminPanelOpen && permissions.canSystemAdmin() && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
